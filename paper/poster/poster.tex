			...
			\end{itemize}
                ...
                \headerbox{Morphological transducers}{name=morphtrans,below=kyrgyz}{
                        ...
                        \begin{itemize}
                                ...
                                \item Also provides a wrapper around popular FST free/open-source
...
%		}
		...
		%\headerbox{Morphophonology}{name=morphophon,below=morphtrans}{
			...
			\begin{itemize}
				\item Some combinations of possessive and case morphemes are distinct
				\item Trade-off:
				\begin{itemize}
...
\vspace{-2em}
\setlength{\columnsep}{1.75em}
\begin{multicols}{3}
\begin{tabbing}
  \texttt{{\small <n>}}\hspace{3.0em} \=  Noun\\
  \texttt{{\small <np>}} \> Proper noun\\
  \texttt{{\small <v>}} \>  Verb\\
  \texttt{{\small <det>}} \>  Determiner\\
  \texttt{{\small <cnjcoo>}} \> Coord. conjunct.\\
  \texttt{{\small <cnjadv>}} \> Adv. conjunct.\\
  \texttt{{\small <adv>}} \> Adverb\\
  \texttt{{\small <vaux>}} \> Auxiliary verb\\
  \texttt{{\small <cop>}} \> Copula\\
  \texttt{{\small <iv>}} \> Intransitive\\
  \texttt{{\small <tv>}} \> Transitive\\
  \texttt{{\small <p2>}} \> Second person\\
  \texttt{{\small <p3>}} \> Third person\\
  \texttt{{\small <ant>}} \> Anthroponym\\
  \texttt{{\small <dem>}} \> Demonstrative\\
  \texttt{{\small <m>}} \> Masculine\\
  \texttt{{\small <sg>}} \> Singular\\
  \texttt{{\small <pl>}} \> Plural\\
  \texttt{{\small <nom>}} \> `Nominative'\\
  \texttt{{\small <gen>}} \> Genitive\\
  \texttt{{\small <acc>}} \> Accusative\\
  \texttt{{\small <loc>}} \> Locative\\
  \texttt{{\small <px3sg>}} \> 3rd person poss. (Singular)\\
  \texttt{{\small <px3pl>}} \> 3rd person poss. (Plural)\\
  \texttt{{\small <neg>}} \> Negative\\
  \texttt{{\small <aor>}} \> Aorist\\
  \texttt{{\small <imp>}} \> Imperative\\
  \texttt{{\small <gna\_perf>}} \> Verbal adverb (Perfect)\\
  \texttt{{\small <prc\_impf>}} \> Participle (Imperfect)\\
  \texttt{{\small <prc\_irre>}} \> Participle (Irrealis)\\
  \texttt{{\small <prc\_real>}} \> Participle (Realis)\\
...
\begin{itemize}
	...
	\item {\texttt \{L\}} desonorises to д after cons.\ of equal or lower sonority\\
...
{\small \texttt{\%\{y\%\}:Vy <=> [ :LastVowel :Cns* :Cns ]/[:0] \blank{1em}\\
	\vspace{1pt}\hspace{1pt}\hfill{} [ :Cns [ .\#.\ | :Cns ] ]/[ :0 | \%>:]\ ;\\
